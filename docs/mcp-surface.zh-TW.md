...
v0.7 新增十二個工具（`import_asset` 到 `validate_pack_asset`；`palette_asset`、`material_asset`、`preview_asset`、`animate_asset` 把 CLI 子命令合併為 `mode` 欄位），達成與 CLI 完全對等。名稱與輸入形狀在此凍結；執行中的伺服器將在下一步註冊它們。
