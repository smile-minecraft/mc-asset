...
当前版本（`v0.3.0`）新增十二个工具（`import_asset` 到 `validate_pack_asset`；`palette_asset`、`material_asset`、`preview_asset`、`animate_asset` 把 CLI 子命令合并为 `mode` 字段），达成与 CLI 完全对等。名称与输入形状在此冻结；运行中的服务器已提供全部十九个工具。
